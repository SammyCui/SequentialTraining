...
def parse_args():
    ...
    parser.add_argument('--download-dir', type=str, default='/Users/xuanmingcui/Documents/projects/cnslab/cnslab/SequentialTrainingCNN/datasets', help='dataset directory on disk')
...
if __name__ == '__main__':
    ...
    if not os.path.isdir(path) or not os.path.isdir(os.path.join(path, 'VOC2007')) \
        or not os.path.isdir(os.path.join(path, 'datasets/VOC2012')):
        if args.no_download:
            raise ValueError(('{} is not a valid directory, make sure it is present.'
                              ' Or you should not disable "--no-download" to grab it'.format(path)))
        else:
            download_voc(path, overwrite=args.overwrite)
            shutil.move(os.path.join(path, 'VOCdevkit', 'datasets/VOC2012'), os.path.join(path, 'datasets/VOC2012'))
    ...
    # make symlink
    makedirs(os.path.expanduser('/u/erdos/students/xcui32/cnslab/datasets'))
