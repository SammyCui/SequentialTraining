...
from data_utils import GenerateBackground, CIFARLoader
...
class RunModel:
    ...
    def load_datasets(self):
        ...
        for target_distances in self.testing_distances:
            ...
            test_dataset = ResizingCIFAR10(root=self.testing_root_path,
                                           transform=transform,
                                           loader=test_loader,
                                           download=True,
                                           train=False)
        ...
        training_len = len(ResizingCIFAR10(root=self.training_root_path, train=True, download=True))
        ...
        for fold in range(self.n_folds):
            ...
            for target_distance in self.target_distances:
                loader = CIFARLoader(self.size, p=target_distance,
                                     background_generator=self.background,
                                     resize_method=self.resize_method)
                dataset = ResizingCIFAR10(root=self.training_root_path,
                                          transform=transform,
                                          train=True,
                                          download=True,
                                          loader=loader)
            ...
            if self.training_mode == 'stb_endsame':
                ...
                for i in range(len(self.target_distances)):
                    ...
                    target_loader = CIFARLoader(self.size, p=self.target_distances[i],
                                                background_generator=self.background,
                                                resize_method=self.resize_method)
                    target_dataset = ResizingCIFAR10(root=self.training_root_path,
                                                     transform=transform,
                                                     train=True,
                                                     download=True,
